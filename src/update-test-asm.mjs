...
async function mainAsync() {
  ...
  let asmEntries = [...asms.entries()].sort(([aAssembly, _aBytes], [bAssembly, _bBytes]) => {
  ...
  for (let [i, [_assembly, bytes]] of asmEntries.entries()) {
    outLines.push(`constexpr U8 listing_${i}[] = {${makeCxxNumberLiterals(bytes)}};`);
  }
  outLines.push("");
  outLines.push("std::unordered_map<std::string_view, std::span<const U8>> asm_listings = {");
  ...
  outLines.push('    std::fprintf(stderr, "error: could not find machine code for assembly:\\n");');
  outLines.push('    std::fprintf(stderr, "  %s\\n", assembly);');
  outLines.push('    std::fprintf(stderr, "Please re-run update-test-asm.mjs to update test-asm-generated.cpp.\\n");');
